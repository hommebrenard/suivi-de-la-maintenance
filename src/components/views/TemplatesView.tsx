...
import { LayoutTemplate, Lock, Search, Plus } from 'lucide-react';
import { WorkOrderTemplate } from '../../types';

interface TemplatesViewProps {
  templates: WorkOrderTemplate[];
  onAddTemplate: (tmpl: Omit<WorkOrderTemplate, 'id'>) => void;
}
    ...
    <div className="flex-1 bg-white min-h-screen flex flex-col">
      ...
      <div className="px-6 py-5 border-b border-gray-200">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 tracking-tight">Modèles d'ordre de travail</h1>
            <p className="text-sm text-gray-500 mt-1">
              Modèles réutilisables pour créer des ordres de travail récurrents.
            </p>
          </div>
      ...
      <div className="flex-1 p-6 bg-gray-50/30">
        {filtered.length === 0 ? (
          /* Empty state matching Screenshot 10 */
          <div className="border-2 border-dashed border-gray-200 rounded-xl p-12 text-center bg-white my-6 max-w-4xl mx-auto shadow-2xs">
            ...
            <h3 className="text-lg font-semibold text-gray-900">Aucun modèle pour le moment</h3>
            <p className="text-sm text-gray-500 mt-1 max-w-md mx-auto">
              Les modèles vous permettent de créer des ordres de travail récurrents avec des détails prédéfinis.
            </p>
            <button
              onClick={() => setIsModalOpen(true)}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-5xl mx-auto">
            {filtered.map(tmpl => (
              <div key={tmpl.id} className="bg-white p-5 rounded-xl border border-gray-200 shadow-2xs space-y-3">
                <div className="flex items-start justify-between">
                  ...
                  <span className="text-xs px-2 py-0.5 bg-blue-50 text-blue-700 rounded font-medium">
                    {tmpl.tasksCount} étapes
                  </span>
                </div>
                <p className="text-xs text-gray-600 line-clamp-2">{tmpl.description}</p>
                <div className="text-xs text-gray-400 pt-2 border-t flex justify-between">
                  <span>Temps estimé: {tmpl.estimatedHours}h</span>
                  <span>Priorité: {tmpl.priority}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full p-6 space-y-4">
            <h3 className="text-lg font-bold text-gray-900">Nouveau modèle d'ordre de travail</h3>

            <form onSubmit={handleSubmit} className="space-y-4 text-sm">
              <div>
                <label className="block text-xs font-semibold text-gray-700 uppercase mb-1">Titre du modèle *</label>
                <input
                  type="text"
                  required
                  placeholder="Ex: Maintenance système hydraulique"
                  value={title}
              ...
              <div>
                ...
                <textarea
                  rows={3}
                  placeholder="Instruisez les opérations à réaliser..."
                  value={description}
